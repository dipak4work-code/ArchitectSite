...
type Post = {
  ...
  category: string; image: string; readTime: string; published: boolean; createdAt: string
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    let posts = await readStore<Post>('blog')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const posts = await readStore<Post>('blog')
    const newPost: Post = {
      id: generateId(), title, excerpt, author, category,
      image: body.image || '', readTime: body.readTime || '5 min read',
      published: body.published || false, createdAt: new Date().toISOString(),
    }
    posts.unshift(newPost)
    await writeStore('blog', posts)
