...
type Project = {
  ...
  description: string; images: string[]; featured: boolean; createdAt: string
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    let projects = await readStore<Project>('projects')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newProject: Project = {
      id: generateId(), title, category, location, description,
